...
class KNIFEArgs:
    ...
    stopping_criterion: Literal["max_epochs", "early_stopping"] = (
        "early_stopping"  # "max_epochs" or "early_stopping" #attention
    )
    n_epochs: int = 25 # CONDITIONNAL : 5 or more (25 for some embedders) is enough to be stable  #attention | was 8 before
    n_epochs_marg: int = 25 # MARGINAL : 25 is enough to be stable | was 4 before #attention
    eps: float = 1e-3
    n_epochs_stop: int = 2 # was 1
    average: str = "var"
    cov_diagonal: str = "var"
    cov_off_diagonal: str = ""

    optimize_mu: bool = False # Was False before
    cond_modes: int = 8 # was 8 before
    marg_modes: int = 8 # was 8 before
    use_tanh: bool = True
    init_std: float = 0.01
