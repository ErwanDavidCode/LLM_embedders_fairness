...
- ✅ **Multi-model comparison**: 15+ embedding models (MPNet, MiniLM, e5, UAE, etc.)
- ✅ **Flexible datasets**: Support for tabular (ACS) and text-based data
- ✅ **Scalable architecture**: Designed for 500 to 100,000+ samples
- ✅ **GPU-optimized**: SLURM-compatible for HPC clusters
- ✅ **Comprehensive pipeline**: Embeddings → (Clustering) → Fairness Scoring → Correlations
...
│   │   ├── PUMS_Data_Dictionary_2018.csv
│   │   ├── 100_000/                    # Full dataset (100k samples)
│   │   │   ├── ACSemployment_with_target.csv
│   │   │   ├── ACSincome_with_target.csv
│   │   │   └── ACSpublic_coverage_with_target.csv
│   │   └── 500/                        # Small test dataset
│   │       └── ACSincome_with_target.csv
│   │
│   └── embeddings/                     # Generated embeddings (output of Step 1)
│       └── 100_000/
│           └── ACSEmployment/
│               ├── MPNet.csv
│               ├── MiniLM-L12.csv
...
│   └── ... (15 models total)
│
├── embedders_fairness/main/            # Main pipeline directory
│   │
│   ├── embedders.json                  # Model registry (paths to local models)
│   ├── run_all_steps_global.sh         # ⭐ MAIN PIPELINE SCRIPT
│   │
│   ├── completion_flags/               # Pipeline execution tracking
│   │   ├── step1_embeddings/
│   │   ├── step1_add_data/
│   │   ├── step2_clustering/
│   │   └── step3_score/
│   │
│   ├── run_embeddings/                 # STEP 1: Embedding generation
│   │   ├── run_all_embeddings.sh       # Embed tabular data manager
|   |   |── run_all_embeddings_text.sh  # Embed text data manager
|   |   |── run_single_embedding.sh     
|   |   |── run_single_embedding_text.sh 
│   │   ├── single_embedding.py         
│   │   └── single_embedding_text.py    
│   │
│   ├── run_add_data/                   # (Optional) Step 1b: Add additional sensitive attributes
│   │   ├── run_all_add_data.sh
│   │   ├── single_add_data_to_embeddings.py
│   │   └── run_single_add_data.sh
│   │
│   ├── run_clusters/                   # STEP 2: Clustering (for unsupervised settings only)
│   │   ├── run_all_clustering.sh       # Cluster embeddings (k-means)
│   │   ├── clustering.py               
│   │   └── run_single_clustering.sh
│   │
│   ├── emir/                           # STEP 3: Fairness Scoring
│   │   ├── emir/                       # Core scoring module
│   │   │   ├── single_score_ISDarrin.py
│   │   │   ├── single_score_supervised_fairentropy.py
│   │   │   ├── single_score_supervised_fairis.py
│   │   │   └── single_score_unsupervised_fairentropy.py
|   |   ├── run_score/                   # Bash module to run core module
│   │   │   ├── run_all_score_ISDarrin.sh
│   │   |   ├── run_all_score_supervised_fairentropy.sh
│   │   |   ├── run_all_score_supervised_fairis.sh
│   │   |   └── run_all_score_unsupervised_fairentropy.sh
│   │   │
│   │   └── ... (the rest of the github of Darrin)
│   │
│   ├── correlations/                   # STEP 4: Analysis & Visualization
│   │   ├── correlation_WGA_*.py        # Correlation analysis scripts
│   │   ├── final_plot.py               # Final visualization
│   │   ├── run_correlation_*.sh        # Correlation executors
│   │   └── additional_scripts/         # Plotting helpers
│   │
│   ├── other_custom_embedders_in_litterature/
│   │   ├── AE_embedding.py             # AutoEncoder embeddings
│   │   └── Tab_embedding.py            # Tabular embeddings
│   │
│   └── pre-pipeline_functions/
│       ├── download_models.py          # Download pre-trained models
│       └── generate_data_ACS_folktable.py  # Generate ACS datasets
│
└── __results/                          # Pipeline outputs (created during execution)
    ├── pipeline_*.out / .err           # SLURM logs
    ├── seed1/K2/                       # Results organized by seed & K
    │   ├── clustering/
    │   ├── score/
    │   └── correlations/
    └── seed1/KSEX_RAC1P/              # Alternative: grouping by sensitive attributes
```
...
[STEP 1] run_embeddings/
    ↓ Generate embeddings with 15 models
